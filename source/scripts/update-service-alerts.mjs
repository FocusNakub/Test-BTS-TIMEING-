...
const sources = [
  ...
  {
    name: "ข่าวนวัตกรรมขนส่งเดลินิวส์",
    lineIds: [],
    query: 'site:facebook.com/TransportDailynews (รถไฟฟ้า OR BTS OR MRT OR Airport Rail Link OR แอร์พอร์ตเรลลิงก์) (ขัดข้อง OR ล่าช้า OR งดให้บริการ OR หยุดเดินรถ)',
  },
];
...
for (const source of sources) {
  try {
    ...
    for (const item of items) {
      ...
      if (!resolvedWords.test(text) || !Number.isFinite(publishedAt.getTime())) continue;
      if (now - publishedAt.getTime() > maxAgeMs) continue;
      for (const lineId of lineIdsFor(source, text)) {
        resolvedAt.set(lineId, Math.max(resolvedAt.get(lineId) || 0, publishedAt.getTime()));
      }
    }
    for (const item of items) {
